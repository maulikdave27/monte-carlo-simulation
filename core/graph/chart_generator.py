...
def plot_sector_comparison(user_sector_weights, optimal_sector_weights):
    """
    Generates a grouped bar chart comparing sector allocations.
    
    Args:
        user_sector_weights (dict): {Sector: Weight} for user
        optimal_sector_weights (dict): {Sector: Weight} for optimal
    """
    # Get all unique sectors sorted
    all_sectors = sorted(list(set(list(user_sector_weights.keys()) + list(optimal_sector_weights.keys()))))
    
    user_vals = [user_sector_weights.get(s, 0) for s in all_sectors]
    opt_vals = [optimal_sector_weights.get(s, 0) for s in all_sectors]
